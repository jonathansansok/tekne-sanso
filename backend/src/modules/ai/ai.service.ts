...
export class AiService {
  ...
  async insights(input: { filters?: { status?: string; policy_type?: string; q?: string } }) {
    ...
    if (nearMin.length > 0) {
      risk_flags += 1
      insights.push(`Hay ${nearMin.length} pólizas con insured_value cerca del mínimo ( < 1.1x ).`)
      insights.push(`Recomendación: generar alerta cuando insured_value < 1.1x del mínimo por tipo.`)
    }
    ...
    if (byTypeEntries.length > 0) {
      ...
      if (top) {
        risk_flags += 1
        insights.push(`Concentración de premium en ${top[0]}: ${Number(top[1]).toFixed(2)} USD.`)
        insights.push(`Recomendación: revisar exposición por tipo y diversificación del portafolio.`)
      }
    }

    if (total >= 100) {
      risk_flags += 1
      insights.push(`Volumen alto (>=100). Recomendación: monitoreo de outliers de premium y asegurado.`)
    }
