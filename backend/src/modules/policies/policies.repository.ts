//backend\src\modules\policies\policies.repository.ts
...
export class PoliciesRepository {
  ...
  async summary(params: { status?: string; policy_type?: string; q?: string }) {
    ...
    return {
      ...
      count_by_status: Object.fromEntries(
        byStatus.map((x) => [x.status, x._count._all]),
      ),
      premium_by_type: Object.fromEntries(
        byType.map((x) => [x.policy_type, x._sum.premium_usd ?? 0]),
      ),
    };
  }
  async createManyStrictInsert(
    rows: Array<{
    ...
    const created = await prisma.policy.createMany({
      data: rows.map((r) => ({
        ...
        end_date: r.end_date,
        premium_usd: new Prisma.Decimal(r.premium_usd),
        status: r.status,
        insured_value_usd: new Prisma.Decimal(r.insured_value_usd),
      })),
      skipDuplicates: true,
    });
